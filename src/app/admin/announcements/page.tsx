...
function AnnouncementsManagementContent() {
  ...
  const fetchAnnouncements = async () => {
    try {
      setLoading(true);
      const fetchedAnnouncements = await getAnnouncements(false); // Get all announcements including unpublished
  ...
  const handleDelete = async (id: string, title: string) => {
    if (
      !confirm(
        `"${title}" başlıklı duyuruyu silmek istediğinizden emin misiniz?`
      )
    ) {
      return;
    }
          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-lg text-gray-300">
                Duyurularınızı ekleyin, düzenleyin veya silin
              </p>
            </div>
            <Link
